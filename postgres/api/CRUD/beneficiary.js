...
module.exports = {
    ...
    postBeneficiary: async (req, res) => {
        ...
        try {
            let { state, district, block, village, first_name, email, mobile } = req.body
            if (state && district && block && village && first_name && email && mobile) {
                let Benef = await benef.create({ state, district, block, village, first_name, email, mobile, user_id })
